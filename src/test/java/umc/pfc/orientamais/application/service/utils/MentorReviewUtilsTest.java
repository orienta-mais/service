...
class MentorReviewUtilsTest {
  ...
  @InjectMocks private MentorReviewUtils mentorReviewUtils;

  private MentorReview review(
      int didactics, int subject, int punctuality, int communication, int engagement) {
  ...
  void calculateSummaryShouldComputeAveragesCorrectly() {
    MentorReviewSummary summary =
        mentorReviewUtils.calculateSummary(List.of(review(5, 4, 3, 2, 1), review(4, 4, 4, 4, 4)));
    ...
    assertEquals(
        (5 + 4 + 3 + 2 + 1 + 4 + 4 + 4 + 4 + 4) / (2.0 * 5.0), summary.getOverallFeedback());
  }
}
