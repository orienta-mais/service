...
class EmailSenderServiceTest {
    ...
    void sendEmailShouldThrowInternalErrorWhenMessagingFails() {
        try (MockedConstruction<MimeMessageHelper> helperConstruction = mockHelper(mock -> {
            try {
    ...
    void sendEmailWithAttachmentShouldAttachFileAndSend() throws Exception {
        ...
        try (MockedConstruction<MimeMessageHelper> helperConstruction = mockHelper()) {
            emailSenderService.sendEmailWithAttachment("destino@teste.com", "Assunto", "<p>Conteúdo</p>", "cert.pdf", fileData);

            MimeMessageHelper helper = helperConstruction.constructed().getFirst();
            ArgumentCaptor<InputStreamSource> resourceCaptor = ArgumentCaptor.forClass(InputStreamSource.class);
    ...
    void sendEmailWithAttachmentShouldThrowInternalErrorWhenMessagingFails() {
        try (MockedConstruction<MimeMessageHelper> helperConstruction = mockHelper(mock -> {
            try {
                doThrow(new MessagingException("fail")).when(mock).addAttachment(anyString(), any(InputStreamSource.class));
            } catch (MessagingException e) {
                throw new RuntimeException(e);
            }
        })) {
            InternalErrorException exception = assertThrows(InternalErrorException.class,
                    () -> emailSenderService.sendEmailWithAttachment("destino@teste.com", "Assunto", "conteúdo", "cert.pdf", new byte[0]));
    ...
    private MockedConstruction<MimeMessageHelper> mockHelper(Consumer<MimeMessageHelper> configurator) {
