...
class MentoredTest {
  ...
  @InjectMocks private Mentored mentoredController;
  ...
  void setUp() {
    ...
    updateRequest =
        new MentoredUpdateModelRequest(
            "Updated", "Mentored", LocalDate.of(1998, 3, 15), "social", "description", "SP", "BR");
  ...
  void shouldPropagateExceptionWhenValidateEmailRequestIsNull() {
    doThrow(new NullPointerException("request"))
        ...
        .validateAndSendLink(null, AuthUserRole.MENTORED);
  ...
  void shouldThrowWhenValidateEmailFails() {
    doThrow(new InvalidOrExpiredTokenException())
        .when(validateEmailUseCase)
        .validateAndSendLink(emailRequest, AuthUserRole.MENTORED);

    assertThrows(
        InvalidOrExpiredTokenException.class, () -> mentoredController.validateEmail(emailRequest));
    verify(validateEmailUseCase).validateAndSendLink(emailRequest, AuthUserRole.MENTORED);
  }
  ...
  void shouldPropagateExceptionWhenRegisterRequestIsNull() {
    doThrow(new NullPointerException("request"))
        ...
        .register(null, AuthUserRole.MENTORED);
  ...
  void shouldThrowWhenRegisterFailsDueToExistingEmail() {
    doThrow(new EmailAlreadyExistsException("Email already exists"))
        .when(registerUseCase)
        .register(registerRequest, AuthUserRole.MENTORED);

    assertThrows(
        EmailAlreadyExistsException.class, () -> mentoredController.register(registerRequest));
    verify(registerUseCase).register(registerRequest, AuthUserRole.MENTORED);
  }
  ...
  void shouldPropagateExceptionWhenGettingAllMentoredsFails() {
    ...
    RuntimeException exception =
        assertThrows(RuntimeException.class, () -> mentoredController.getAllMentoreds());
  ...
  void shouldPropagateExceptionWhenGetMentoredByIdFails() {
    ...
    RuntimeException exception =
        assertThrows(RuntimeException.class, () -> mentoredController.getMentoredById(mentoredId));
  ...
  void shouldUpdateMentoredSuccessfully() {
    when(mentoredUseCase.updateMentored(mentoredId, updateRequest))
        .thenReturn(mentoredModelResponse);

    ResponseEntity<MentoredModelResponse> response =
        mentoredController.updateMentored(mentoredId, updateRequest);
  ...
  void shouldPropagateExceptionWhenDeleteMentoredFails() {
    ...
    RuntimeException exception =
        assertThrows(RuntimeException.class, () -> mentoredController.deleteMentored(mentoredId));
  ...
  void shouldGetMentorInfosSuccessfully() {
    ...
    ResponseEntity<MentorInfoModelResponse> response =
        mentoredController.getMentorInfosById(mentorId);
  ...
  void shouldPropagateExceptionWhenGetMentorInfosFails() {
    ...
    RuntimeException exception =
        assertThrows(RuntimeException.class, () -> mentoredController.getMentorInfosById(mentorId));
  ...
  void shouldAddMentorReviewSuccessfully() {
    ...
    ResponseEntity<GenericModelResponse> response =
        mentoredController.addMentorReview(mentorId, mentorReviewRequest);
  ...
  void shouldPropagateExceptionWhenMentorReviewRequestIsNull() {
    when(mentorReviewUseCase.addMentorReview(mentorId, null))
        .thenThrow(new NullPointerException("request"));

    assertThrows(
        NullPointerException.class, () -> mentoredController.addMentorReview(mentorId, null));
