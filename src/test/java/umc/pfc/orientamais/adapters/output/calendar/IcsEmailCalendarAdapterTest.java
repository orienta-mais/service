...
class IcsEmailCalendarAdapterTest {
    ...
    @InjectMocks
    private IcsEmailCalendarAdapter adapter;
    ...
    void shouldCreateEventAndSendInvite() throws Exception {
        ...
        try (MockedStatic<IcsBuilder> icsBuilder = mockStatic(IcsBuilder.class);
             MockedConstruction<MimeMessageHelper> helperConstruction = mockConstruction(MimeMessageHelper.class)) {
            icsBuilder.when(() -> IcsBuilder.generateUid(lesson)).thenReturn("UID-123");
            icsBuilder.when(() -> IcsBuilder.buildIcsEvent(eq(lesson), anyList(), anyString(), anyString()))
    ...
    void shouldThrowRuntimeExceptionWhenMailSenderFails() throws Exception {
        ...
        try (MockedStatic<IcsBuilder> icsBuilder = mockStatic(IcsBuilder.class);
            ...
            RuntimeException exception = assertThrows(RuntimeException.class, () ->
                    adapter.createEvent(lesson, List.of("user@email.com")));
    ...
    void shouldSendInviteToMentoredSuccessfully() throws Exception {
        ...
        try (MockedStatic<IcsBuilder> icsBuilder = mockStatic(IcsBuilder.class);
            ...
            icsBuilder.when(() -> IcsBuilder.buildIcsEvent(eq(lesson), eq(List.of("mentored@email.com")), anyString(), anyString()))
                    .thenReturn("BEGIN:VCALENDAR");
