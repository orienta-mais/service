...
public class CertificatePdfGenerator {
    ...
    private static final String CERTIFICATE_VALIDATION_URL = "https://orienta.org/certificados/validar/";
    ...
    public byte[] generateCertificate(Mentored mentored, Lesson lesson) {
        try {
            ...
            Font titleFont = new Font(Font.FontFamily.HELVETICA, 32, Font.BOLD, new BaseColor(25, 25, 25));
            Paragraph title = new Paragraph("Certificado de Participação", titleFont);
            title.setAlignment(Element.ALIGN_CENTER);
            title.setSpacingBefore(80);
            document.add(title);

            Font subtitleFont = new Font(Font.FontFamily.HELVETICA, 15, Font.NORMAL, new BaseColor(100, 100, 100));
    ...
    private void addBorder(PdfContentByte canvas) {
        Rectangle outer = new Rectangle(18, 18, PageSize.A4.rotate().getWidth() - 18, PageSize.A4.rotate().getHeight() - 18);
        outer.setBorder(Rectangle.BOX);
        outer.setBorderWidth(2);
        outer.setBorderColor(new BaseColor(200, 200, 200));
        canvas.rectangle(outer);

        Rectangle inner = new Rectangle(28, 28, PageSize.A4.rotate().getWidth() - 28, PageSize.A4.rotate().getHeight() - 28);
