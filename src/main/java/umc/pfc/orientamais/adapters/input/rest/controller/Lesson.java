...
public class Lesson {
  ...
  @PostMapping
  public ResponseEntity<GenericModelResponse> createLesson(
      @Valid @RequestBody CreateLessonModelRequest request) {
    GenericModelResponse response = lessonUseCase.createLesson(request);
    return ResponseEntity.status(HttpStatus.CREATED).body(response);
  }

  @GetMapping("/{lessonId}")
  public ResponseEntity<LessonDetailsModelResponse> listLessonById(
      @Valid @PathVariable String lessonId) {
    LessonDetailsModelResponse response = lessonUseCase.listLessonById(lessonId);
    return ResponseEntity.status(HttpStatus.OK).body(response);
  }

  @PutMapping("/{lessonId}")
  public ResponseEntity<GenericModelResponse> updateLesson(
      @PathVariable String lessonId, @Valid @RequestBody UpdateLessonModelRequest request) {
  ...
  public ResponseEntity<List<LessonDetailsModelResponse>> listLessonById(
      @Valid @PathVariable UUID mentorId) {
  ...
  public ResponseEntity<List<LessonDetailsModelResponse>> listLessonByMentoredId(
      @Valid @PathVariable UUID mentoredId) {
  ...
  public ResponseEntity<PagedModelResponse<LessonModelResponse>> listLessons(
    ...
    PagedModelResponse<LessonModelResponse> response =
        lessonUseCase.listLesson(title, date, order, page, size);
    return ResponseEntity.ok(response);
  }

  @PostMapping("/{lessonId}/certificate/validate")
  public ResponseEntity<GenericModelResponse> validatePresence(
      @PathVariable UUID lessonId, @Valid @RequestBody PresenceCodeModelRequest request) {
    GenericModelResponse response =
        certificateUseCase.validatePresenceAndGenerateCertificate(lessonId, request);
    return ResponseEntity.status(HttpStatus.CREATED).body(response);
  }
