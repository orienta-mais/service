...
$$
    DECLARE
        ...
        cities               TEXT[]    := ARRAY [
            ...
            'Curitiba - PR','Recife - PE','Fortaleza - CE','Salvador - BA','Manaus - AM','Belém - PA',
            'Goiânia - GO','Campinas - SP','Niterói - RJ','Joinville - SC','São José dos Campos - SP'
            ];
    ...
    BEGIN
        ...
        FOR i IN 1..50
            LOOP
                ...
                VALUES (v_user, format('mentor.%s@example.com', i), '$2a$10$DEMO_HASHED_PASSWORD_PLACEHOLDER', 'MENTOR')
        ...
        FOR i IN 1..300
            LOOP
                v_user := uuid_generate_v4();
                INSERT INTO auth_user (id, email, password, role)
                VALUES (v_user, format('mentorado.%s@example.com', i), '$2a$10$DEMO_HASHED_PASSWORD_PLACEHOLDER',
                        'MENTORED')
                ON CONFLICT (email) DO NOTHING;
